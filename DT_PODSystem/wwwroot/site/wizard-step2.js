// wizard-step2.js - Template Details Step (Updated with File Name Prefix)
// ========================================================================

// Initialize Step 2
function initializeStep2() {
    console.log('🟢 Initializing Step 2: Template Details');
...
// Save Step 2 data to database (ONLY called on Next button)
async function saveStep2Data() {
    try {
        const step2Data = getStep2FormData();

        console.log('💾 [STEP2] Saving step 2 data:', step2Data);

        const response = await fetch('/Template/SaveStep2', {
            method: 'POST',
            headers: {
        ...
        if (result.success) {
            console.log('💾 [STEP2] ✅ Step 2 saved successfully');
            return true;
        } else {
            console.error('💾 [STEP2] ❌ Save failed:', result.message);
            alert.error(result.message || 'Failed to save step 2 data');
            return false;
        }
    } catch (error) {
        console.error('💾 [STEP2] ❌ Save error:', error);
        alert.error('Error saving step 2 data');
        return false;
    }
}
...
// Setup event handlers for Step 2 (ONLY for UI updates, NO saving)
...
function loadServerData() {
    const serverData = window.serverWizardData?.Step2;
    ...
    if (serverData) {
        ...
        console.log('📥 Loaded Step 2 data from server (fixed)');
    }
}

// Get Step 2 form data for saving (FIXED - Save only prefix)
function getStep2FormData() {
    ...
    console.log('🔍 [DEBUG] Collecting Step 2 form data...');
...
// Custom validation for Step 2 (called on Next button)
...
window.debugStep2Values = function () {
